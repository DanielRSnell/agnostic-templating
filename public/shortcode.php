...
if ( ! function_exists( 'agnostic_shortcode' ) ) :

  function agnostic_shortcode( $atts, $content = null, $tag, $class = null ) {
    ...
    if ( empty( $atts['id'] ) ) {
      ...
      if ( empty( $atts['conditions'] ) && $tag !== NULL ) {
        return sprintf( '<p>%s</p>', __( 'Please select a template to render.', 'agnostic-templates' ) );
      ...
      $autoload = false;
        ...
        $source = agnostic_allowed_data_sources( get_post_meta( get_the_ID(), '_agnostic_data_source', true ) );

        $code = agnostic_kses( get_post_meta( get_the_ID(), '_agnostic_template_code', true ) );

        $placement = get_post_meta( get_the_ID(), '_agnostic_attribute_placement', true );

				$matched_conditions = get_post_meta( get_the_ID(), '_agnostic_condition', false );
        ...
        if ( $source == 'single' ) {
          ...
          $data_query = (array) get_post_meta( get_the_ID(), '_agnostic_data_query', true );
        ...
        $code = Timber::compile_string( $code, $context );
				...
				do_action( 'agnostic_rendering', array(
